...
Another useful function is =partial=. It's called such because it partially fills in the arguments for an existing function, producing a new function of fewer arguments in the process. For example, Clojure includes a function called =inc= that adds one to the number you pass in, so that =(inc 1)= gives you 2 and =(inc 41)= is 42. It's easy enough to make your own version of =inc=:
...
Another handy function-producing functino that comes packaged with Clojure is =complement=. With =complement=, every day is opposite day. It wraps the function that you supply with a call to =not=, producing a function that is the complement of th eoriginal. For example, earlier we wrote =adventure?=, which could tell adventure books from those of other genres.

#+begin_src clojure
(defn adventure? [book]
  (when (= (:genre book) :adventure)
    book))
(defn not-adventure? (complement adventure?))
#+end_src

Last one--=every-pred= combines predicate functions into a single function that /ands/ them all together. With =every-pred= we can dispense with our home-grown =both-f=:
...
** Function Literals
Another way that Clojure comes to your aid in creating new functions is to supply an alternate, minimalistic syntax for defining them. So for those moments wh en even the sleek lines of =fn= seem like too much syntactical overhead, you can use a function literal: just a =#= followed by the function body, wrapped in the usual parens. Here, for example, are the guts of =adventure?= recast as a function literal.

#+begin_src clojure
#(when (= (:genre %1) :adventure) %1)
#+end_src

There are no named arguments in function literals. Instead, they use the shell-script notation of =%1= to stand in for the first argument, =%2= for the second, and so forth. So if we needed a function that would double a numver, we might use =partial= or we'd do this:
...
There are a few things to keep in mind about function literals, or /lambdas/, as they are known. First, function literals and =fn= produce exactly the same kind of thing; the only difference is the syntax.

Second, remember that Clojure infers the numver of arguments that your literally defined function takes from the highest-numbered argument in the function body. Thus, if we modified our /double the number/ function into this:

#+begin_src clojure
#(* 2 %11)
#+end_src

We'd end up with a function that takes 11 arguments and ignores the first 10.

Finally, function literals have a special feature aimed directly at the very common case of creating a one-argument function. If the functioni you're building takes a single argument, you can use plain old =%=, without a numver, for the one and only argument.
